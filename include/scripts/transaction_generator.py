...
def generate_transaction_data(user_id, num_transactions, data_interval_start):
    ...
    for _ in range(num_transactions):
        if data_interval_start is not None:
            transaction_date = data_interval_start + timedelta(hours=random.randint(0, 23),
...
def main(data_interval_start=None):
    ...
    for user_id in range(1, num_users + 1):
        transactions = generate_transaction_data(user_id, random.randint(5, 20),
                                                 data_interval_start)  # Each user gets 5 to 20 transactions
